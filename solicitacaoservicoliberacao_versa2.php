...
function Botao2()
{
document.nome_formulario.action="novasolicitacaoservicoliberacao_versa2.php";
document.forms.nome_formulario.submit();
}
</script>
...
<form action="solicitacaoservicoliberacao_versa2.php" method="POST" name="nome_formulario">
...
?>
<center><font style='font-size: 24px;color: red;text-align:left;bold: Negrito' >Liberação de Acesso</font></br>
<font  style='font-size: 13px;color: black;text-align:center;bold: Negrito' >


Classificar Pessoas que Irão Requisitar<select name="tiposolicita" style="background-color:#8B8378;color: white;">
    <option value="unida">Funcionários da Empresa</option>
    <option value="forne">Funcionários de Fornecedores</option>
    </select>

Empresa Solicitante: <select name="empresasolicita" style="background-color:#8B8378;color: white;">
                 <option value="<?ECHO $empresainicial;?>"><?ECHO $DESCRICAOempresainicial;?></option>
              	 <?
                 ...
                 (SELECT cd_pessoa from usuario_grupo_bi where cd_usuario='$usuario')";
				  $res = mysql_db_query($banco,$stmt1,$conexao);
                    ...
                    while ($valor9=mysql_fetch_array($res)) {
					 print "<option value=\"{$valor9["CODIGO"]}\">{$valor9["DESCRICAO"]}</option>";

					}
				 ?>
				 </select>
				 
				 <input  type="submit" VALUE="Mostrar Registro">


	Classificar Pessoas de Acesso<select name="tipoexecutor" style="background-color:#8B8378;color: white;">
    <option value="unida">Funcionários da Unidade</option>
    <option value="forne">Funcionários de Fornecedores</option>
    </select>
...
<? 
       ...
       $s = OCIParse($ora_conexao, "select  DISTINCT a.NR_PEDIDO NR_PEDIDO,g.DATA_DIGITACAO DATA1,g.DATA_EMISSAO EMISSAO,
                                  a.COD_FORNECEDOR  FORNE,d.ds_motivo MOTIVO,
                                  a.CD_EMPRESA  EMPRESA1,h.nome QUEMSOLICITOU,h.cd_sec SECAOSOLICITOU,
    ...
    While (OCIFetch($s)) {
      ...
      $secaosolicitante=ociresult($s, "SECAOSOLICITOU") ;
      $observacaosolicitante=ociresult($s, "OBSERVA") ;


      IF ($codigofornecedor=="99999"){$liberdopara=$solicitante;}else{$liberdopara=$descricaofornecedor;}
          ...
          ?>
			<tr>
               ...
               if (($status=="ABER")){
                  ?>
              <td BGCOLOR=<?echo $cor;?>><a href="liberarservicomaximiza.php?pagina=1&escolha=1&tela=1&mes=<?echo $mes;?>&ano=<?echo $ano;?>
...
&pedido=<?echo $pedido;?>&secaosolicitante=<?echo $secaosolicitante;?>"><font style='font-size: 9px;color: black;text-align:left;bold: Negrito' >
