...
<form action="TRANSFERIRMAQUINA.php"  name="formulario" method="POST">
...
$semanainicio=$_POST['semanainicio'];
 ...
 ?>
<CENTER><font style='font-size: 19px;color: BLACK;text-align:left;bold: Negrito' >Transferência com Saida de Equipamentos </font></BR>
...
Etapa-1 Buscar Equipamentos</br></br></br>
                 ...
                 Unidade Negócio de Saida:<select name="fabricapesquisa">
                 <?
                 ...
                 and d.cd_centro in (select distinct CD_CENTRO FROM ATUACAO_USUARIO_CENTRO WHERE
                 CD_USUARIO='$codigousuario' and id_atualizar='S')
                 ";
...
<tr>
				...
				<th style='font-size: 9px;text-align:left;bold: Negrito'>Selecionar</th>
				 ...
				 <?
				 echo "select CD_CENTRO,DS_CENTRO FROM CENTRO_TRABALHOS  WHERE  CD_CENTRO IN ( SELECT CD_CENTRO FROM
		...
		<input type="hidden" name="centrodestinofinal" value="<?echo $centrodestinofinal;?>">
...
if ($escolha=="avancando2"){
 ...
 for ($passa=0;$passa <=$contadorarea;$passa++)
 {
  ...
  cd_maquina not in (select cd_maquina from MOVIMENTACAO_MAQUINA_BI where status='T') and
  cd_centro in (select cd_centro from ATUACAO_USUARIO_CENTRO where ID_ATUALIZAR='S' and  CD_USUARIO='$usuariopesquisa') and
  cd_maquina in (select cd_maquina from MOVIMENTACAO_MAQUINA_BI where STATUS='A')
  ");
        ...
        While (OCIFetch($sselecao)) {
          ...
          $descricao4=ociresult($sselecao, "CD_CENTRO") ;
               ...
               ?>
					...
					<td ><input type="checkbox" name="lista[]" value="<?echo $descricao1;?>"></td>
...
echo "vai comecar a gravar//contador:$contadorcentro";
  ...
  for ($passa=0;$passa <=$contadorcentro;$passa++)
 {
                           ...
                           $centrodestinofinal2=$centrodestinofinal[$passa];

                           if ($criafinal>"0"){
                           
                           
                           $maquina17 = OCIParse($ora_conexao, "select CD_CENTRO FROM MAQUINAS WHERE CD_MAQUINA='$criafinal'");
                         OCIExecute($maquina17, OCI_DEFAULT);
                         While (OCIFetch($maquina17)) {

                           $centrosaoda=ociresult($maquina17, "CD_CENTRO") ;
                         ...
                         While (OCIFetch($quantoss17)) {
                           ...
                           echo "UPDATE  MOVIMENTACAO_MAQUINA_BI  SET CD_CENTRO_SAIDA='$centrosaoda',
                           CD_USUARIO_LIBERACAO='$usuariopesquisa',DT_SAIDA='$datapesquisa1',STATUS='T' WHERE
                           CD_MAQUINA='$criafinal'";



                           $sgrava1= OCIParse($ora_conexao, "UPDATE  MOVIMENTACAO_MAQUINA_BI  SET CD_CENTRO_SAIDA='$centrosaoda',
                           CD_USUARIO_LIBERACAO='$usuariopesquisa',DT_SAIDA='$datapesquisa1',STATUS='T' WHERE
                           CD_MAQUINA='$criafinal'");
                           OCIExecute($sgrava1, OCI_DEFAULT);
                           oci_commit($ora_conexao);
