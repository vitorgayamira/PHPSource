...
<form action="classificacaoepi.php"  name="formulario" method="POST">
...
<CENTER><font style='font-size: 19px;color: BLACK;text-align:left;bold: Negrito' >Classificação - EPI </font></BR>



<a href="classificacaoepi.php?pagina=1&escolha=novoregistro&tela=0&mes=<?echo $mes;?>&ano=<?echo $ano;?>&turnopesquisa=<?echo $turnopesquisa;?>&curva=<?echo $curva;?>&codigousuario=<?echo $codigousuario;?>&codigoempresa=<?echo $codigoempresa;?>&navegando=sim&banco=<?echo $banco;?>
...
<a href="classificacaoepi.php?codigousuario=<?echo $codigousuario;?>&codigoempresa=<?ECHO $codigoempresa;?>
...
<a href="classificacaoepi.php?pagina=1&escolha=novoregistro&tela=1&mes=<?echo $mes;?>&ano=<?echo $ano;?>
...
<?
 ...
 $s = OCIParse($ora_conexao, "select CD_CLASSIFICACAO,DS_CLASSIFICACAO FROM classificacao_epi ORDER  by DS_CLASSIFICACAO ASC");
        OCIExecute($s, OCI_DEFAULT);
        While (OCIFetch($s)) {

          $codigo=ociresult($s, "CD_CLASSIFICACAO") ;
          $descricao=ociresult($s, "DS_CLASSIFICACAO") ;
...
<tr>
				...
				<th style='font-size: 9px;text-align:left;bold: Negrito'>Selecionar</th>
                 ...
                 ?>
					...
					<td><?echo $row["DS_CLASSIFICACAO"];?></td>
					<td ><input type="checkbox" name="listafamilia[]" value="<?echo $row["CD_CLASSIFICACAO"];?>"></td>
                    </tr>
                 <?
...
if ($escolha=="3"){
                 ...
                 $result2 = "select MAX(CD_CLASSIFICACAO) SERVICO FROM
                 classificacao_epi";
                 $result3 = @OCIParse($ora_conexao, $result2);
				 @OCIExecute($result3, OCI_DEFAULT);
				 while (OCIFetchInto($result3,&$row, OCI_ASSOC+OCI_RETURN_NULLS))
                 ...
                 $sgrava1= OCIParse($ora_conexao, "insert into classificacao_epi(CD_CLASSIFICACAO,DS_CLASSIFICACAO)
...
if ($escolha=="4"){//ELIMINANDO
  ...
  if ($criafinal>"0"){
                           ...
                           $sgrava1= OCIParse($ora_conexao, "DELETE FROM classificacao_epi  WHERE
                           CD_CLASSIFICACAO='$criafinal'");
