...
<form action="historicoterceiro.php"  name="nome_formulario" id="nome_formulario" method="POST" >
...
$sgrava1= OCIParse($ora_conexao, "delete from AUX_GRAFICO_TERCEIRO1");
...
<center><font style='font-size: 29px;color: red;text-align:left;bold: Negrito' >Serviços Terceiros</font></center></br>
...
<a href="historicoterceiro.php?navegando=sim&escolha=3&statusnovapessoa=nao&acaoparametro=selecionar&iniciotela=sim&banco=<?echo $banco;?>&datainicialmostra=<?echo $datainicialmostra;?>&datafinalmostra=<?echo $datafinalmostra;?>&bday1=<?echo $datainicialmostra;?>&bday2=<?echo $datafinalmostra;?>">
<IMG src="pesquisarnovocva.png" width="42" height="38" ></a>



<a href="historicoterceiro.php?navegando=sim&escolha=4&statusnovapessoa=nao&acaoparametro=selecionar&iniciotela=sim&banco=<?echo $banco;?>&datainicialmostra=<?echo $datainicialmostra;?>&datafinalmostra=<?echo $datafinalmostra;?>&bday1=<?echo $datainicialmostra;?>&bday2=<?echo $datafinalmostra;?>">
<IMG src="graficonovo.png" width="42" height="38" ></a>

<a href="historicoterceiro.php?navegando=sim&escolha=5&statusnovapessoa=nao&acaoparametro=selecionar&iniciotela=sim&banco=<?echo $banco;?>&datainicialmostra=<?echo $datainicialmostra;?>&datafinalmostra=<?echo $datafinalmostra;?>&bday1=<?echo $datainicialmostra;?>&bday2=<?echo $datafinalmostra;?>">
<IMG src="graficonovo2.png" width="42" height="38" ></a>
...
$grupomostra="Todas";
   ...
   IF ($grupoordens=="T"){
                       ...
                       SELECT DISTINCT FS.COD_FORNECEDOR EMPRESA, STI.NR_PEDIDO PEDIDO,STI.DATA_ENTREGA ENTREGA, STI.NR_ORDEM ORDEM, STI.QUANTIDADE QUANT,
                       STI.VL_UNITARIO VALOR,STI.ITEM ITEM, STI.DS_SERVICO SERVICO,
           STI.TERMINO_GARANTIA GARANTIA, OC.CD_TIPO TIPO,FS.EMPRESA RAZAO,CF.DATA_BAIXA_INDIVIDUAL DATABAIXA
     ...
     WHERE STB.NR_ORDEM = OC.NUMERO_ORDEM
       ...
       union
       SELECT DISTINCT FS.COD_FORNECEDOR EMPRESA, STI.NR_PEDIDO PEDIDO,
           STI.DATA_ENTREGA ENTREGA, STI.NR_ORDEM ORDEM, STI.QUANTIDADE QUANT, STI.VL_UNITARIO VALOR, STI.ITEM ITEM, STI.DS_SERVICO SERVICO,
           STI.TERMINO_GARANTIA GARANTIA, EC.CD_TIPO TIPO,FS.EMPRESA RAZAO,CFE.DATA_BAIXA_INDIVIDUAL DATABAIXA
 ...
 IF ($grupoordens=="C"){
                       ...
                       $s = OCIParse($ora_conexao, "SELECT DISTINCT FS.COD_FORNECEDOR EMPRESA, STI.NR_PEDIDO PEDIDO,
           STI.DATA_ENTREGA ENTREGA, STI.NR_ORDEM ORDEM, STI.QUANTIDADE QUANT, STI.VL_UNITARIO VALOR, STI.ITEM ITEM, STI.DS_SERVICO SERVICO,
           STI.TERMINO_GARANTIA GARANTIA, EC.CD_TIPO TIPO,FS.EMPRESA RAZAO,CFE.DATA_BAIXA_INDIVIDUAL DATABAIXA
      FROM SERV_TERC_ITENS STI, SERV_TERC_PEDIDO STP, EMER_COMP EC, FABRICAS FA,
           AREAS AR, SECAOS SE, CENTRO_TRABALHOS CT, COMPONENTE_AUXILIAR CA,
 ...
 $s = OCIParse($ora_conexao, "
     ...
     WHERE STB.NR_ORDEM = OC.NUMERO_ORDEM
      ...
      /*
      echo "SELECT DISTINCT FS.COD_FORNECEDOR, STI.NR_PEDIDO,STI.DATA_ENTREGA, STI.NR_ORDEM, STI.QUANTIDADE,
                       STI.VL_UNITARIO,STI.ITEM, STI.DS_SERVICO,
           STI.TERMINO_GARANTIA, OC.CD_TIPO
      FROM SERV_TERC_BAIXA STB, SERV_TERC_ITENS STI, SERV_TERC_PEDIDO STP, ORDEM_COMP OC,
           FABRICAS FA, AREAS AR, SECAOS SE, CENTRO_TRABALHOS CT, COMPONENTE_AUXILIAR CA,
           MAQUINAS MA, FORNECEDOR_SADEGE FS, COMP_FUNC CF
     WHERE STB.NR_ORDEM = OC.NUMERO_ORDEM
       ...
       AND TRUNC(CF.DATA_BAIXA_INDIVIDUAL) >= '$datapesquisa1'
       AND TRUNC(CF.DATA_BAIXA_INDIVIDUAL) <= '$datapesquisa2'
       AND OC.CD_TIPO LIKE '%$tipo%'
       AND STP.COD_FORNECEDOR = FS.COD_FORNECEDOR
       AND STP.CGC = FS.CGC
       AND FS.COD_FORNECEDOR LIKE '%$fornecedor%'
       AND OC.NUMERO_ORDEM LIKE '%$ordem%'
       AND OC.CD_COMPONENTE = CA.CD_COMPO
       AND CA.CD_COMPO LIKE '%$componente%'
       AND CA.CD_MAQUINA = MA.CD_MAQUINA
       AND MA.CD_MAQUINA LIKE '%$maquina%'
       AND MA.CD_CENTRO = CT.CD_CENTRO
       AND CT.CD_SECAO = SE.CD_SECAO
       AND SE.CD_AREA = AR.CD_AREA
       AND AR.CD_AREA LIKE  '%$area%'
       AND AR.CD_FABRICA = FA.CD_FABRICA
       AND FA.CD_FABRICA LIKE '%$fabricapesquisa%'
       union
       SELECT DISTINCT FS.COD_FORNECEDOR, STI.NR_PEDIDO,
           STI.DATA_ENTREGA, STI.NR_ORDEM, STI.QUANTIDADE, STI.VL_UNITARIO, STI.ITEM, STI.DS_SERVICO,
           STI.TERMINO_GARANTIA, EC.CD_TIPO
      FROM SERV_TERC_ITENS STI, SERV_TERC_PEDIDO STP, EMER_COMP EC, FABRICAS FA,
           AREAS AR, SECAOS SE, CENTRO_TRABALHOS CT, COMPONENTE_AUXILIAR CA,
           MAQUINAS MA, FORNECEDOR_SADEGE FS, COMP_FUNC_EMER CFE
     WHERE STI.NR_ORDEM = EC.NUMERO
...
<tr>
    ...
    $total=$quant2*$valor2;
      ?>
      <tr class="difr">
       <td><? echo ociresult($s, "RAZAO");?></a></td>
       <td><? echo ociresult($s, "PEDIDO");?></td>
       <td><? echo ociresult($s, "ITEM");?></td>
       <td><? echo ociresult($s, "SERVICO");?></td>
       <td><? echo ociresult($s, "ORDEM");?> </td>
       <td><? echo ociresult($s, "TIPO");?> </td>
       <td><? echo ociresult($s, "ENTREGA");?> </td>
       <td><? echo ociresult($s, "GARANTIA");?> </td>
       <td><? echo ociresult($s, "QUANT");?> </td>
       <td><? echo ociresult($s, "VALOR");?> </td>
       <td><? echo $total;?> </td>
       <?
       $razao3=ociresult($s, "RAZAO");
       $databaixaachou1=ociresult($s, "DATABAIXA");
       $mesachou2=substr($databaixaachou1,3,2);
       

       $sgrava166= OCIParse($ora_conexao, "INSERT INTO AUX_GRAFICO_TERCEIRO1(CD_MES,
                                  DS_EMPRESA,VL_CUSTO )
                                  VALUES ('$mesachou2','$razao3','$total')");
  ...
  <?
                                ...
                                $achapart5= OCIParse($ora_conexao, "SELECT SUM(VL_CUSTO) VALOR  FROM AUX_GRAFICO_TERCEIRO1");
                                ...
                                $achapart6= OCIParse($ora_conexao, "SELECT CD_MES,DS_EMPRESA,VL_CUSTO
                                FROM AUX_GRAFICO_TERCEIRO1");
                                OCIExecute($achapart6, OCI_DEFAULT);
                                While (OCIFetch($achapart6))
                                {
                                  $custoindi=ociresult($achapart6, "VL_CUSTO") ;
                                  $mesindi=ociresult($achapart6, "CD_MES") ;
                                  ...
                                  $sgrava166= OCIParse($ora_conexao, "update AUX_GRAFICO_TERCEIRO1
                                  set vl_participacao=$achoupart66
                                  where cd_mes='$mesindi' and ds_empresa='$empresaindi'
                                  ");
...
<div  style="display: block;" id="mostraterceiros">
          ...
          <td align=right>
           ...
           <a href="historicoterceiro.php?navegando=sim&escolha=1&statusnovapessoa=nao&acaoparametro=selecionar&iniciotela=sim&banco=<?echo $banco;?>&bday1=<?echo $datainicialmostra;?>&bday2=<?echo $datafinalmostra;?>">
...
?>
 ...
 <?require("historicoterceirograf.php");?>
 </div>
...
?>
 ...
 <table width="99%" >
          ...
          <tr>
          <td><titulo>Custo Total Valores(R$)</titulo></td>
 ...
 <?require("historicoterceirograf2.php");?>
 </div>
