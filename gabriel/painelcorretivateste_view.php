...
<style  type="text/css">
 ...
 #div_1 { position: relative; width: 650px; height: 1200px; background-color: #CAE1FF; border:1;float: left; text-align: center; color: #E8E8E8; z-index:-1; }
 #div_2 { position: relative; width: 650px; height: 1200px; background-color: #CAE1FF; float: right; text-align: center; color: #E8E8E8; z-index:-1;}
 #clear { clear:both; }
...
<meta http-equiv="refresh" content="30;URL=painelcorretivateste_view.php?banco=<?echo $banco;?>&codigousuario=<?echo $codigousuario;?>&localbanco=<?echo $localbanco;?>">
...
<?
 ...
 $codigoproximoregitro1 = "SELECT  count(*) CONTADOR
                      ...
                      b.cd_mao='MEC' and fabrica='$fabricapesquisa' and andamento  not in ('FINALIZADO')";
$codigoproximoregitro2= $conn->Execute($codigoproximoregitro1);
while ( !$codigoproximoregitro2->EOF)
{
...
<div id="div_1">
 ...
 <?
                      ...
                      FROM   AUX_PAINEL_CORRETIVA_W WHERE NRORDEM in (select NUMERO FROM EMER_COMP WHERE CD_MAO='MEC') and fabrica='$fabricapesquisa'
...
$codigoproximoregitro1 = "SELECT  count(*) CONTADOR
                      FROM   AUX_PAINEL_CORRETIVA_W a,EMER_COMP b
                      where a.NRORDEM=b.numero and
                      b.cd_mao='ELE' and fabrica='$fabricapesquisa' and andamento  not in ('FINALIZADO')";
$codigoproximoregitro2= $conn->Execute($codigoproximoregitro1);
while ( !$codigoproximoregitro2->EOF)
{
$totalordens=$codigoproximoregitro2->fields["0"];
...
<div id="div_2">
 ...
 <?
                      ...
                      MAQ_PARADA PARA,STATUS_ANDAMENTO_TIPO OBSERVACAO ,DATA_INICIO DATA,CD_COMPONENTE ,
                      NOME,MOTIVO,DESCRICAOCOMPONENTE
                      FROM   AUX_PAINEL_CORRETIVA_W WHERE NRORDEM in (select NUMERO FROM EMER_COMP WHERE CD_MAO='ELE') and fabrica='$fabricapesquisa'
                      and andamento  not in ('FINALIZADO')  ORDER BY NRORDEM ASC";
                      $achaordens2= $conn->Execute($achaordens1);
                      while ( !$achaordens2->EOF)
                      {
