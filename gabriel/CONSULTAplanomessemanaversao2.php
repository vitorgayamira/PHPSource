...
function Botao2()
{
document.nome_formulario.action="CONSULTAplanomessemana.php";
...
<form action="CONSULTAplanomessemanaversao2.php"  name="nome_formulario" id="nome_formulario" method="POST" >
...
if (!IsSet($escolha)){
    ...
    $achaMINIMOMES2= mysql_db_query($banco,$achaMINIMOMES,$conexao);
    $achaMINIMOMES3=mysql_fetch_array($achaMINIMOMES2);
    $mesinicio=$achaMINIMOMES3["MESINICIO"];
    $mesfinal=$achaMINIMOMES3["MESFINAL"];
    ...
    $achadianovo2dd77 = mysql_db_query($banco,$achadianovo1dd77,$conexao);
    $achadianovo3dd77=mysql_fetch_array($achadianovo2dd77);
    $primeirodia=$achadianovo3dd77["DIA1"];
    ...
    $achadianovo2dd88 = mysql_db_query($banco,$achadianovo1dd88,$conexao);
    $achadianovo3dd88=mysql_fetch_array($achadianovo2dd88);
    $ultimodia=$achadianovo3dd88["DIA2"];
...
else
{
   ...
   from aux_semana a,meses_bi b
    ...
    ELSE
    {
     ...
     $achadianovo1dd= "SELECT NR_DIA_INICIAL DIA1,NR_DIA_FINAL DIA2,NR_MES,NR_SEMANA SEMANA from aux_semana
...
<select name="semanapegou" style="background-color:#8B8378;color: white;" onchange="this.form.submit()">
                 ...
                 <?
                 $stmt199 = "SELECT distinct a.NR_SEMANA SEMANA
                 from aux_semana a,meses_bi b
                 ...
                 a.nr_dia_final <=b.qt_dias
                  ORDER BY a.nr_sequencia asc";
					 ...
					 print "<option value=\"{$valor1["SEMANA"]}\">{$valor1["SEMANA"]}</option>";
					 }
					 ?>
     				 ...
     				 $sachafabricades = OCIParse($ora_conexao, "SELECT DS_FABRICA FROM   FABRICAS
                      WHERE CD_FABRICA='$fabricapesquisa'");
                      OCIExecute($sachafabricades, OCI_DEFAULT);
                      While (OCIFetch($sachafabricades)) {
                          $fabricapesquisades=ociresult($sachafabricades, "DS_FABRICA") ;
...
<select name="fabricapesquisa" style="background-color:#8B8378;color: white;" onchange="this.form.submit()">
				 ...
				 ?>
                     ...
                     <?
                      $sachaequide = OCIParse($ora_conexao, "SELECT DS_COMPO FROM   COMPONENTE_AUXILIAR
                      WHERE CD_COMPO='$equipamentoescolha'");
                      OCIExecute($sachaequide, OCI_DEFAULT);
                      While (OCIFetch($sachaequide)) {
                          $componentedescricao=ociresult($sachaequide, "DS_COMPO") ;

                      }
                   ?>

    <select name="equipamentoescolha" style="background-color:#8B8378;color: white;" onchange="this.form.submit()">
                 <option value="<?echo $equipamentoescolha;?>">Equipamento:<? echo $equipamentoescolha;?>-
                 <? echo $componentedescricao;?></option>
...
}//final quano o mesincial=mesfinal
...
<tr>
 ...
 for ($passa12=$contadoriteminicial2;$passa12 <=30;$passa12++){
...
if ($dia=="Dom")
{
	  ?><td><h1 style='font-size: 9px;color: red;text-align:left;bold: Negrito text-decoration: none'><?echo $dia;?></br><?echo $passa12;?></td>
<?
}
else
{
?><td><h1 style='font-size: 9px;color: black;text-align:left;bold: Negrito text-decoration: none'><?echo $dia;?></br><?echo $passa12;?></td>
<?
...
if ($dia=="Dom")
{
	  ?><td><h1 style='font-size: 9px;color: red;text-align:left;bold: Negrito text-decoration: none'><?echo $dia;?></br><?echo $passa13;?></td>
<?
}
else
{
?><td><h1 style='font-size: 9px;color: black;text-align:left;bold: Negrito text-decoration: none'><?echo $dia;?></br><?echo $passa13;?></td>
<?
...
for ($passa6=$contadoriteminicial;$passa6 <=30;$passa6++){
