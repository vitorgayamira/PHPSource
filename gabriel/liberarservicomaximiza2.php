...
<form action="liberarservicomaximiza2.php"  name="nome_formulario"  method="POST" >
...
$semanainicio=$_POST['semanainicio'];
	...
	$diainicial=$_POST['diainicial'];
		...
		$datagravarinicio=$_POST['datagravarinicio'];
            ...
            $secaosolicitante==$_POST['secaosolicitante'];
            $pedidoaltera==$_POST['pedidoaltera'];
...
$today = date('d/m/y H:i:s'); 
          $acha166s= "SELECT CODIGO from pessoa_bi where CD_UNIDANEGOCIO_ERP='$secaosolicitante'";
          ...
          WHERE CD_USUARIO='$codigousuario' and
          cd_pessoa='$pessoa5'";
...
?>
<center>
Seção Solicitante:<?echo $secaosolicitante;?>-Pedido:<?echo $pedidoaltera;?>
...
<?
    ...
    $result2achaorigemfuncioanrio1= "select count(*) CONTADOR from
                                  solicitacao_motivo where nr_pedido='$pedidoaltera' and COD_FORNECEDOR not  in('99999')";
                                  ...
                                  solicitacao_motivo where nr_pedido='$pedidoaltera' ";
 ...
 IF ($contador66 =="0"){
 $s= OCIParse($ora_conexao, "select a.nr_sequencia SEQUENCIA,b.CADASTRO CADASTRO,b.NOME NOME,
 a.DT_LIMITE_ACESSO_INICIAL DATA1,
 a.DT_LIMITE_ACESSO_FINAL DATA2, a.id_liberado APRO FROM
 SOLICITACAO_MOTIVO_FUNC a, FUNCIONARIO_SADEGE b
 where b.cd_sec='$secaosolicitante' and
 a.cd_funcionario=b.cadastro and
 a.nr_pedido='$pedidoaltera'
              ");
 ...
 where
 a.cd_funcionario=b.cadastro and
 a.nr_pedido='$pedidoaltera' AND
 b.cd_fornecedor='$pegaforne'
              ");
          ...
          $sequenciaS=ociresult($s, "SEQUENCIA") ;
          $data1S=ociresult($s, "DATA1") ;
          $data2S=ociresult($s, "DATA2") ;
   ...
   <?if (($data1S==null) or ($data2S==null)){?>
   <td><input type="txt" name="data1[]"  maxlength="10"></td>
   <td><input type="txt" name="data2[]"  maxlength="10"><input type="HIDDEN" name="lista[]" value="<?echo $sequenciaS;?>"></td>
 ...
 <input type="hidden" name="ano"  value="<?echo $ano;?>" >
  ...
  <input type="hidden" name="segundasemana" value="<?echo $segundasemana;?>">
		...
		<input type="hidden" name="secaosolicitante" value="<?echo $secaosolicitante;?>">
...
if ($escolha=="atualizar5") {
    ...
    else
    {
     echo "Existe Pedido ";
                                  ...
                                  FROM solicitacao_motivo where nr_pedido='$pedidoaltera'";
                                  ...
                                  $result2xistevei89 = "select count(*) CONTADOR  FROM
                                  SOLICITACAO_MOTIVO_FUNC a, FUNCIONARIO_SADEGE b
                                  where b.cd_sec='$secaosolicitante' and  a.cd_funcionario=b.cadastro and
                                  a.nr_pedido='$pedidoaltera' and
                                  ((a.DT_LIMITE_ACESSO_INICIAL=null) or (a.DT_LIMITE_ACESSO_FINAL=null))
                                  ";
                                  ...
                                  for ($passa=0;$passa <=$ultimo;$passa++)
                                  {
                                   ...
                                   $statusgrav5="FUNC";
                                     ...
                                     $sgrava1= OCIParse($ora_conexao, "update  SOLICITACAO_MOTIVO_FUNC set
                                     DT_LIMITE_ACESSO_INICIAL='$datainicial' ,DT_LIMITE_ACESSO_FINAL='$datafinal'
                                     where nr_sequencia='$acao' ");
                                  ...
                                  $result2xistevei89zz = "select count(*) CONTADOR  FROM
                                  SOLICITACAO_MOTIVO_FUNC a, FUNCIONARIO_SADEGE b
                                  where b.cd_sec='$secaosolicitante' and  a.cd_funcionario=b.cadastro and
                                  a.nr_pedido='$pedidoaltera' and
                                  ((a.DT_LIMITE_ACESSO_INICIAL=null) or (a.DT_LIMITE_ACESSO_FINAL=null))
                                  ";
                                  $result3ver89zz = @OCIParse($ora_conexao, $result2xistevei89zz);
				                  @OCIExecute($result3ver89zz, OCI_DEFAULT);
				                  while (OCIFetchInto($result3ver89zz,&$row, OCI_ASSOC+OCI_RETURN_NULLS))
				                  {
                                    $quantosnullainda=ociresult($result3ver89zz, "CONTADOR") ;
                                  }

                                  if ($quantosnullainda=="0"){
                                  
                                    $sgrava1= OCIParse($ora_conexao, "update  SERV_TERC_PEDIDO
                                     set ID_STATUS='FUNC' WHERE  nr_pedido='$pedidoaltera' ");
                                     OCIExecute($sgrava1, OCI_DEFAULT);
                                     oci_commit($ora_conexao);
