...
<form action="solicitacaoservicoliberacao2.php" method="POST" name="nome_formulario">
...
?>
<center><font style='font-size: 24px;color: red;text-align:left;bold: Negrito' >Autorização de Acesso</font></br>
                 ...
                 (SELECT cd_pessoa from usuario_grupo_bi where cd_usuario='$usuario')";
				  $res = mysql_db_query($banco,$stmt1,$conexao);
                    ...
                    while ($valor9=mysql_fetch_array($res)) {
					 print "<option value=\"{$valor9["CD_UNIDANEGOCIO_ERP"]}\">{$valor9["DESCRICAO"]}</option>";
...
<? 
        ...
        else
        {
         $s = OCIParse($ora_conexao, "select  DISTINCT a.NR_PEDIDO NR_PEDIDO,g.DATA_DIGITACAO DATA1,g.DATA_EMISSAO EMISSAO,
                                  ...
                                  a.cd_servico=c.cd_servico and
                                  c.CD_FUNC_SOLICITANTE=h.cadastro and
                                  h.cd_sec in ('$empresasolicita') and
                                  a.nr_pedido=g.nr_pedido and
                                  g.id_status  in('FUNC') AND
                                  c.cd_motivo=d.cd_motivo
                                  ");
      ...
      IF ($codigofornecedor=="99999"){$liberdopara=$solicitante;}else{$liberdopara=$descricaofornecedor;}
          ...
          ?>
			<tr>
              ...
              if (($status=="FUNC")){?>
              <td BGCOLOR=<?echo $cor;?>><a href="solicitacaoservicoliberacao2.php?pagina=1&escolha=liberarfuncionario&tela=1&mes=<?echo $mes;?>&ano=<?echo $ano;?>
...
&pedidoaltera=<?echo $pedido;?>&secaosolicitante=<?echo $secaosolicitante;?>&localbanco=<?echo $localbanco;?>"><font style='font-size: 9px;color: black;text-align:left;bold: Negrito' >
