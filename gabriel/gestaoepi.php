...
<form action="gestaoepi.php" method="POST" name="nome_formulario">
...
if (!IsSet($anopesquisa)){
 ...
 <center><font style='font-size: 24px;color: red;text-align:left;bold: Negrito' >Definição e Controle de EPI</font></br>
 ...
 <input type="button" onclick="Botao2()" value="Definir Novo EPI"></input>

<?
if (!IsSet($escolha) or ($escolha=="1") or ($escolha=="gravandoorcamento") or ($escolha=="aprovar") ){

//valida o status de epi
$s88= OCIParse($ora_conexao, "select  a.cd_epi EPI,a.DT_VENCIMENTO+a.VL_TOLERANCIA_USO DATA,
a.VL_TOLERANCIA_USO TOLERANCIA,
 a.cd_item ITEM
 FROM epi a,mat_pecas b,mat_pecas_fabrica c
 WHERE a.cd_item=b.cd_material and
 b.cd_material=c.cd_material and
 c.cd_fabrica='$solicitantepesquisa'
 ");
 OCIExecute($s88, OCI_DEFAULT);
 While (OCIFetch($s88)) {
      $codigo=ociresult($s88, "EPI") ;
      $data27=ociresult($s88, "DATA") ;
      $tolerancia2=ociresult($s88, "TOLERANCIA") ;
      


      $anopegou=substr($data27,6,2);
      $mespegou=substr($data27,3,2);
      $diapegou=substr($data27,0,2);
   ...
   if ($anopegou < $anotoday){
          $sgrava18= OCIParse($ora_conexao, "update  EPI set id_status='VE' where CD_EPI='$codigo'");
...
<center><table class=MsoTableGrid border=1 cellspacing=0 cellpadding=0
 ...
 <tr>
                ...
                <th>Data </br>Liberação Uso</th>
                <th>Status CA</th>
                <th>Máximo </br>Uso(Dias)</th>
...
<? 
 ...
 a.cd_unidade_item=e.cd_unidade  ORDER BY a.DT_VENCIMENTO ASC
 ");
 ...
 While (OCIFetch($s)) {
      ...
      $anopegouVENCI=substr($data28,6,2);
      $mespegouVENCI=substr($data28,3,2);
      $diapegouVENCI=substr($data28,0,2);
...
<?
 ...
 a.nr_pedido=d.nr_pedido order  by c.ds_servico,a.VL_UNITARIO asc");
   ...
   if (($status=="ABER") or ($status=="APRO") or ($status=="ANDA") or ($status=="FINA") or ($status=="DESA"))
   {?>
   <td>
   <input type="hidden" name="sequenciatrata[]" value="<?echo $pedidoS;?>" >
   </td>
