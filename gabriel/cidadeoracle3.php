...
<form action="cidadeoracle3.php" method="POST" name="nome_formulario">
...
<center><font style='font-size: 32px;color: red;text-align:center;bold: Negrito' >Atualizando Cidades</font>
</br></br>
 ...
 <td><input type="txt" name="cidadepesquisa"  size="60" ></td>
 </tr>
 ...
 <tr>
 <td>Estado</td>
 <td><select name="estadopesquisa">
                 <option value=""></option>


 <?
				 $result2 = "SELECT CD_ESTADO,SIGLA FROM ESTADO_SADEGE ";
				 ...
				 {
					print "<option value=\"{$row["CD_ESTADO"]}\">{$row["SIGLA"]}</option>";
                 }
				 ?>
				 </select>
...
$result2 = "select MAX(CD_CIDADE) DISTR FROM
                 CIDADE_SADEGE";
                 $result3 = @OCIParse($ora_conexao, $result2);
				 @OCIExecute($result3, OCI_DEFAULT);
				 while (OCIFetchInto($result3,&$row, OCI_ASSOC+OCI_RETURN_NULLS))
                 ...
                 $sgrava1= OCIParse($ora_conexao, "insert into CIDADE_SADEGE(CD_CIDADE,DS_CIDADE,CD_ESTADO)
                 VALUES('$maxima2','$cidadepesquisa','$estadopesquisa')");
