...
$elimina1= OCIParse($ora_conexao, "delete from  AUX_CONSULTA_CUSTO_ATIV");
...
<font style='font-size: 22px;color: RED;text-align:left;bold: Negrito' >Consulta Custo Padrão - Atividades</font>
<hr>
...
<a href="consultamaterias_cheklist2.php?navegando=sim&codigoempresa=<?ECHO $codigoempresa;?>
...
<td>
        ...
        <tr bgcolor="#f5f5f5">
            ...
            <b><? echo "$month_name $year"; ?></b></font>
             ...
             <hr><a href="consultamaterias_cheklist2.php?navegando=sim&codigoempresa=<?ECHO $codigoempresa;?>
        ...
        <?
         ...
         if ($contador=="1"){
               ...
               $programa="consultamaterias_cheklist2.php";
...
//grava a auxiliar
  ...
  $smacha= OCIParse($ora_conexao, "select a.cd_componente CODIGOEQUIPAMENTO,
  ...
  OCIExecute($smacha, OCI_DEFAULT);
  While (OCIFetch($smacha))
  {

          $codigoequipamento=ociresult($smacha, "CODIGOEQUIPAMENTO") ;
          $codigomaquina=ociresult($smacha, "MAQUINA") ;
          $codigoatividade=ociresult($smacha, "ATIVIDADE") ;
          
          $sgrava1= OCIParse($ora_conexao, "insert into AUX_CONSULTA_CUSTO_ATIV(CD_COMPONENTE,VL_CUSTO_MATERIAL,
  ...
  OCIExecute($sm, OCI_DEFAULT);
        While (OCIFetch($sm)) {
          ...
          $codigoatividade=ociresult($sm, "ATIVIDADE") ;

          $valorcusto= str_replace(",", ".","$valorcusto" );



           $sgrava1= OCIParse($ora_conexao, "UPDATE AUX_CONSULTA_CUSTO_ATIV SET VL_CUSTO_MATERIAL=$valorcusto
           WHERE
  ...
  OCIExecute($sm, OCI_DEFAULT);
        While (OCIFetch($sm)) {
          ...
          $valorcustopeca= str_replace(",", ".","$valorcustopeca" );
                     ...
                     /*
                        $quantoss17mat= OCIParse($ora_conexao, "select VL_CUSTO_MATERIAL
                         from AUX_CONSULTA_CUSTO_ATIV
                         WHERE CD_COMPONENTE='$codigoequipamento' AND CD_ATIVIDADE='$codigoatividade'");
                         OCIExecute($quantoss17mat, OCI_DEFAULT);
                         While (OCIFetch($quantoss17mat)) {

                           $customaterial=ociresult($quantoss17mat, "VL_CUSTO_MATERIAL") ;
                         }

                   $custototal=$customaterial+$valorcustopeca;
                   */
          ...
          $sgrava1= OCIParse($ora_conexao, "UPDATE AUX_CONSULTA_CUSTO_ATIV SET VL_CUSTO_PECAS=$valorcustopeca
           WHERE
          CD_COMPONENTE='$codigoequipamento' AND CD_ATIVIDADE='$codigoatividade'");
                           OCIExecute($sgrava1, OCI_DEFAULT);
                           oci_commit($ora_conexao);
  ...
  $achazero1= OCIParse($ora_conexao, "select CD_COMPONENTE,CD_ATIVIDADE,VL_CUSTO_MATERIAL,VL_CUSTO_PECAS from
  ...
  While (OCIFetch($achazero1))
  {
   ...
   $atividade4=ociresult($achazero1, "CD_ATIVIDADE") ;
   ...
   $sgrava1= OCIParse($ora_conexao, "UPDATE AUX_CONSULTA_CUSTO_ATIV SET VL_CUSTO_TOTAL=$custototal16 WHERE
  ...
  $gravapart= OCIParse($ora_conexao, "select CD_COMPONENTE,CD_ATIVIDADE,VL_CUSTO_TOTAL from AUX_CONSULTA_CUSTO_ATIV ");
  ...
  While (OCIFetch($gravapart))
  {
          ...
          $codigoatividade7=ociresult($gravapart, "CD_ATIVIDADE") ;
          ...
          else
          {
            $participacao="0";
                           ...
                           $sgrava1= OCIParse($ora_conexao, "UPDATE AUX_CONSULTA_CUSTO_ATIV
  ...
  <th style='font-size: 11px;color: white;text-align:left;bold: Negrito'>Atividade</th>
  ...
  $smpegaaux= OCIParse($ora_conexao, "select CD_MAQUINA,CD_ATIVIDADE, TO_CHAR(SUM(VL_CUSTO_MATERIAL),'999990.999')
    CUSTO,TO_CHAR(SUM(VL_CUSTO_PECAS),'999990.999') CUSTO2,TO_CHAR(SUM(VL_CUSTO_TOTAL),'999990.999')  CUSTO3,
          TO_CHAR(SUM(VL_PARTICIPACAO),'999990.999')  PART from AUX_CONSULTA_CUSTO_ATIV GROUP BY CD_MAQUINA,CD_ATIVIDADE
          order by PART desc");
  ...
  OCIExecute($smpegaaux, OCI_DEFAULT);
        ...
        $atividade9=ociresult($smpegaaux, "CD_ATIVIDADE") ;
