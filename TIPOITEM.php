...
<form action="TIPOITEM.php"  name="formulario" method="POST">
...
<center><font style='font-size: 39px;color: red;text-align:left;bold: Negrito' >Classificação de Materiais</font></br>
...
&turnopesquisa=<?echo $turnopesquisa;?>&curva=<?echo $curva;?>
&codigousuario=<?echo $codigousuario;?>
&codigoempresa=<?echo $codigoempresa;?>&navegando=sim&banco=<?echo $banco;?>&baseoracle=<?ECHO $baseoracle;?>">
...
<?
 ...
 $s = OCIParse($ora_conexao, "select CD_TIPO,DS_TIPO,CD_UNIDADE FROM TIPO_ITEM ORDER  by DS_TIPO ASC");
        OCIExecute($s, OCI_DEFAULT);
        While (OCIFetch($s)) {

          $codigo=ociresult($s, "CD_TIPO") ;
          $descricao=ociresult($s, "DS_TIPO") ;
          $unid=ociresult($s, "CD_UNIDADE") ;
...
?>
Criando Classificação</br>

Descrição :<input type="txt" name="novogrupo" size="35"  >
Unidade de Uso:  <select name="usounidade">
                 <option value=""></option>
                 <?
				 $result2 = "SELECT CD_UNIDADE,DS_UNIDADE FROM UNIDADES order by CD_UNIDADE ASC";
...
<tr>
				...
				<th style='font-size: 9px;text-align:left;bold: Negrito'>Selecionar</th>
				 ...
				 <?
				 $contadorfamilia="0";
                 $result2 = "select CD_TIPO,DS_TIPO FROM TIPO_ITEM";
				 $result3 = @OCIParse($ora_conexao, $result2);
				 @OCIExecute($result3, OCI_DEFAULT);
				 while (OCIFetchInto($result3,&$row, OCI_ASSOC+OCI_RETURN_NULLS))
				 {
                 ...
                 ?>
					...
					<td ><input type="checkbox" name="listafamilia[]" value="<?echo $row["CD_TIPO"];?>"></td>
                    </tr>
                 <?
...
if ($escolha=="3"){
                 ...
                 $result2 = "select MAX(CD_TIPO) REGIAO FROM
                 TIPO_ITEM";
                 $result3 = @OCIParse($ora_conexao, $result2);
				 @OCIExecute($result3, OCI_DEFAULT);
				 while (OCIFetchInto($result3,&$row, OCI_ASSOC+OCI_RETURN_NULLS))
                 ...
                 $sgrava1= OCIParse($ora_conexao, "insert into TIPO_ITEM(CD_TIPO,DS_TIPO,CD_UNIDADE) VALUES('$maxima2','$novogrupo','$usounidade')");
...
if ($escolha=="4"){//ELIMINANDO
  ...
  if ($criafinal>"0"){
                        ...
                        // }
                           ...
                           IF ($acha55 =="0"){
                           $sgrava1= OCIParse($ora_conexao, "DELETE FROM TIPO_ITEM WHERE CD_TIPO='$criafinal'");
                           OCIExecute($sgrava1, OCI_DEFAULT);
                           oci_commit($ora_conexao);
                           }
