...
?>
		...
		<table>
			...
			<tr>
				...
				<tr>
					...
					<td>
						<?
						 $sqlequipamento = "SELECT DS_COMPO FROM  COMPONENTE_AUXILIAR WHERE CD_COMPO='$equipamentoescolha'";
						 $sqlequipamentoexec = $conn->Execute($sqlequipamento);
						 $equipdescricao=$sqlequipamentoexec->fields["0"];
						 ?>
						 <select name="equipamentoescolha" onchange="this.form.submit()">
						 <option value="<?echo $equipamentoescolha;?>"><? echo $equipamentoescolha;?>-<? echo $equipdescricao;?></option>
						 <option value="">Todos Equipamentos</option>
						 <?
						 $sqllistaequipamento = "SELECT DISTINCT ma.cd_maquina valor
						  ...
						  $sqllistaequipamentoexec = $conn->Execute($sqllistaequipamento);
							 while ( !$sqllistaequipamentoexec->EOF)
									{
									  print "<option value=\"{$sqllistaequipamentoexec->fields["VALOR"]}\">{$sqllistaequipamentoexec->fields["VALOR"]}-{$sqllistaequipamentoexec->fields["MAQUINA"]}</option>";
									  $sqllistaequipamentoexec->MoveNext();
									}
						 ?>
						 </select>
					 </td>
	 ...
	 <tr> 
         ...
         <td>
                        <?
							 $sqlcomponente = "SELECT ds_compo FROM componente_auxiliar WHERE cd_compo = '$componenteescolha' AND flag != 'Q'";
							 $sqlcomponenteexec = $conn->Execute($sqlcomponente);
							 $componentedescricao=$sqlcomponenteexec->fields["0"];

                         ?>
                     <select name="componenteescolha" onchange="this.form.submit()">
                     <option value="<?echo $componenteescolha;?>"><? echo $componenteescolha;?>-<? echo $componentedescricao;?></option>
                     <option value="">Todos Componentes</option>
                    <?
				     $sqllistacomponente = "SELECT DISTINCT ca.cd_compo valor
                     ...
                     $sqllistacomponenteexec = $conn->Execute($sqllistacomponente);
                     while ( !$sqllistacomponenteexec->EOF) {
                          print "<option value=\"{$sqllistacomponenteexec->fields["VALOR"]}\">{$sqllistacomponenteexec->fields["VALOR"]}-{$sqllistacomponenteexec->fields["DESCRICAO"]}</option>";
						  $sqllistacomponenteexec->MoveNext();
                     }
					?>
					</select>
		</td>
    ...
    <tr>
        ...
        <td>
					<?
						 $sqlatividade = "SELECT ds_atividade  FROM atividades WHERE cd_atividade = '$atividadeescolha'";
						 $sqlatividadeexec = $conn->Execute($sqlatividade);
						 $atividadedescricao=$sqlatividadeexec->fields["0"];
					?>
                     <select name="atividadeescolha" onchange="this.form.submit()">
						 <option value="<?echo $atividadeescolha;?>"><? echo $atividadeescolha;?>-<? echo $atividadedescricao;?></option>
						 <option value="">Todas Atividades</option>
						<?
							 $listaatividade = "SELECT  at.cd_atividade valor , at.ds_atividade descricao
												  FROM atividades at
											  ORDER BY at.ds_atividade";
							 $listaatividadeexec = $conn->Execute($listaatividade);
							 while ( !$listaatividadeexec->EOF) 
							 {
								   print "<option value=\"{$listaatividadeexec->fields["VALOR"]}\">{$listaatividadeexec->fields["VALOR"]}-{$listaatividadeexec->fields["DESCRICAO"]}</option>";
								   $listaatividadeexec->MoveNext();
							 }
						?>
