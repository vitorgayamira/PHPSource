...
<form action="cadastrarservicos.php"  name="formulario" method="POST">
...
<CENTER><font style='font-size: 19px;color: BLACK;text-align:left;bold: Negrito' >Cadastro de Serviços de Manutenção </font></BR>



<a href="cadastrarservicos.php?pagina=1&escolha=novoregistro&tela=0&mes=<?echo $mes;?>&ano=<?echo $ano;?>&turnopesquisa=<?echo $turnopesquisa;?>&curva=<?echo $curva;?>&codigousuario=<?echo $codigousuario;?>&codigoempresa=<?echo $codigoempresa;?>&navegando=sim&banco=<?echo $banco;?>
...
<a href="cadastrarservicos.php?codigousuario=<?echo $codigousuario;?>&codigoempresa=<?ECHO $codigoempresa;?>
...
<a href="cadastrarservicos.php?pagina=1&escolha=novoregistro&tela=1&mes=<?echo $mes;?>&ano=<?echo $ano;?>
...
<?
 ...
 $s = OCIParse($ora_conexao, "select CD_SERVICO,DS_SERVICO FROM SERVICO_GERAIS_SADEGE ORDER  by DS_SERVICO ASC");
        OCIExecute($s, OCI_DEFAULT);
        While (OCIFetch($s)) {

          $codigo=ociresult($s, "CD_SERVICO") ;
          $descricao=ociresult($s, "DS_SERVICO") ;
...
<tr>
				...
				<th style='font-size: 9px;text-align:left;bold: Negrito'>Selecionar</th>
				 ...
				 <?
				 $contadorfamilia="0";
                 $result2 = "select CD_SERVICO,DS_SERVICO FROM SERVICO_GERAIS_SADEGE";
                 ...
                 ?>
					...
					<td><?echo $row["DS_SERVICO"];?></td>
					<td ><input type="checkbox" name="listafamilia[]" value="<?echo $row["CD_SERVICO"];?>"></td>
                    </tr>
                 <?
...
if ($escolha=="3"){
                 ...
                 $result2 = "select MAX(CD_SERVICO) SERVICO FROM
                 SERVICO_GERAIS_SADEGE";
                 $result3 = @OCIParse($ora_conexao, $result2);
				 @OCIExecute($result3, OCI_DEFAULT);
				 while (OCIFetchInto($result3,&$row, OCI_ASSOC+OCI_RETURN_NULLS))
				 {
                 ...
                 $sgrava1= OCIParse($ora_conexao, "insert into SERVICO_GERAIS_SADEGE(CD_SERVICO,DS_SERVICO)
...
if ($escolha=="4"){//ELIMINANDO
  ...
  if ($criafinal>"0"){
                         ...
                         from SERVICO_GERAIS_SADEGE WHERE CD_SERVICO='$criafinal'");
                         ...
                         While (OCIFetch($quantoss17)) {
                           ...
                           IF ($acha55 =="0"){
                           $sgrava1= OCIParse($ora_conexao, "DELETE FROM SERVICO_GERAIS_SADEGE  WHERE
                           CD_SERVICO='$criafinal'");
                           OCIExecute($sgrava1, OCI_DEFAULT);
                           oci_commit($ora_conexao);
                           }
